...
function DONATIONRegisterReducer(state = {donation:{}}, action) {
  switch (action.type) {
    case DONATION_SAVE_REQUEST:
      return { loading: true };
    case DONATION_SAVE_SUCESS:
      return { loading: false, success: true, donation: action.payload };
    case DONATION_SAVE_FAIL:
      return { loading: false, error: action.payload };
    default:
      return state;
  }
}
function DONATIONDeleteReducer(
  state = { donation:{} },
  action
) {
  switch (action.type) {
